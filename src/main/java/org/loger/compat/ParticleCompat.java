...
public final class ParticleCompat {
    ...
    private static Particle resolveParticle(String name) {
        ...
        if (adapter.isAtLeast(ServerVersion.V1_20_5)) {
            ...
            if (modernName != null && (modern = tryGetParticle(modernName)) != null) {
                return modern;
            }
            return null;
        }
        String legacyName = MODERN_TO_LEGACY.get(name);
        if (legacyName != null && (legacy = tryGetParticle(legacyName)) != null) {
            return legacy;
        }
        return null;
    }
    ...
    public static Particle getWitchParticle() {
        return getParticle(VersionAdapter.get().isAtLeast(ServerVersion.V1_20_5) ? "WITCH" : "SPELL_WITCH");
    }

    public static Particle getExplosionParticle() {
        return getParticle(VersionAdapter.get().isAtLeast(ServerVersion.V1_20_5) ? "EXPLOSION_EMITTER" : "EXPLOSION_HUGE");
    }

    public static Particle getDustParticle() {
        return getParticle(VersionAdapter.get().isAtLeast(ServerVersion.V1_20_5) ? "DUST" : "REDSTONE");
    }

    public static Particle getEntityEffectParticle() {
        return getParticle(VersionAdapter.get().isAtLeast(ServerVersion.V1_20_5) ? "ENTITY_EFFECT" : "SPELL_MOB");
    }
    ...
    public static void spawnParticle(World world, Particle particle, Location loc, int count, double dx, double dy, double dz, double speed) {
    ...
    public static <T> void spawnParticle(World world, Particle particle, Location loc, int count, double dx, double dy, double dz, double speed, T data) {
        if (particle == null || world == null || loc == null) {
            return;
        }
        try {
